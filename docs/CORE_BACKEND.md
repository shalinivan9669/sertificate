...
The application runs as a Nuxt/Nitro Node function on Vercel Hobby with an explicitly configured remote libSQL/Turso database. On 2026-09-08 the owner confirmed account use and connected Turso database-sky-village to Production and Preview, with preview branches and Development off. Migrations 001–012 and three fresh FK/readiness checks passed. Main commit `a304ae5` was deployed to `https://www.otcenter.kz`; the 07:23:50 UTC public smoke confirmed health/readiness, available Auth configuration with email delivery false, and 20 database-backed consultation directions with request-only prices and zero published versions. See IMPLEMENTATION_STATUS.md for exact deployment, CI and verification limits. Vercel's temporary filesystem is never used as a persistent database: `VERCEL` without a valid remote URL and token produces a 503 response for persistent functions. Public catalog consultation entries remain available without database configuration.
...
This is remote export→LOCAL restore of an initial database. Remote restoration, RTO/RPO, populated production recovery and old-runtime rollback remain unverified. Email is explicitly OFF by owner choice, not an unresolved request for SMTP credentials; verified-email account and reset delivery are not claimed while disabled.

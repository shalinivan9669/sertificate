...
## Точечная устойчивость наблюдателя CI после фактической блокировки SQLite

В GitHub job `101946607904` на commit `4ed` в `2026-09-08 05:27:56 UTC` первые пять attribution checkpoints прошли; шестой остановился на `SQLITE_BUSY` в отдельном тестовом SELECT-наблюдателе при чтении количества путей. Это не было отказом HTTP приложения. Исправлены только два browser harness и общий `tests/helpers/browser-read-observer.mjs`: один SELECT повторяется только при точных `SQLITE_BUSY`/`SQLITE_LOCKED`, максимум восемь вызовов с семью паузами общей длительностью 1 025 мс. При исчерпании повторов выбрасывается та же исходная ошибка; другие ошибки сразу передаются наружу. Записи приложения и HTTP-команды не повторяются. Все прежние проверки значений, количества строк, payload, sidecar и прав сохранены без ослабления.

Три focused tests в `tests/browser-read-observer.test.mjs` фактически прошли, **3/3, exit 0, 1 719.9156 мс**: восстановление после временных BUSY/LOCKED с сохранением точных SQL/args/rows; постоянная блокировка с исходным исключением после восьми вызовов; отсутствие повторов для других SQL errors и запрет write/multistatement. Это детерминированные проверки политики наблюдателя с настоящим типом `LibsqlError`, а не новый нагрузочный тест SQLite. Лог `artifacts/browser-read-observer-focused.log`. Focused ESLint всех четырёх QA-файлов и `node --check` обоих browser harness — exit 0.

Разрешённый повтор actual3111 на том же **Node `daf1207c-adcc-4623-85da-2271911d6fe4`** завершился **10/10 PASS, exit 0, JS errors 0**: `artifacts/lead-attribution-browser/33aed6e7-fb6f-4d29-a011-342262dd0445/report.json`, лог `implementation-b48-attribution-browser-daf1207c-readretry.log`. Оба RU/KK PNG просмотрены. Новый compiled workspace-прогон и новая GitHub job этим исполнителем не запускались; их результаты нельзя выводить из этого локального повторения. Runtime, сборки и прежние артефакты не изменены.

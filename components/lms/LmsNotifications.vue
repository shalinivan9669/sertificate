...
const { data, pending, error, refresh } = await useAsyncData('lms-notifications', () => api<{ notifications: Array<{ id: string; template: string; status: string; createdAt: string }> }>('/me/notifications'));
const busy = ref('');
const failure = ref('');

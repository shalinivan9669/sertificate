...
const courseLink = (courseSlug) => {
  const citySlug = cityParam.value || defaultCitySlug;
  return localePath(`/${citySlug}/${courseSlug}`);
};
...
useHead(() => ({
  ...
  link: [{ rel: 'canonical', href: `${siteUrl}${localePath(post.value?._path || '')}` }],
}));
</script>
...
<template>
  <main v-if="post" class="space-y-8">
    <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <header class="space-y-2 mb-4">
        <p class="text-sm font-semibold text-brand-accent uppercase tracking-wide">Статья</p>
          ...
          <ul class="grid gap-2 md:grid-cols-2">
            <li v-for="course in relatedCourses" :key="course.slug">
              <NuxtLink :to="courseLink(course.slug)" class="text-brand hover:underline">
                {{ course.name[locale] || course.name.ru }}
              </NuxtLink>
            </li>
          </ul>
        </section>
  ...
  <p v-else class="text-slate-700">Публикация не найдена.</p>
</template>

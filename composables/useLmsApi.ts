...
// Decimal input is converted to minor units exactly; this never determines a paid/refunded state.
export function lmsMoneyInputMinor(value: string): number | null {
...
export function useLmsApi() {
  ...
  const errorText = (error: any): string => {
    ...
    const known: Record<string, string> = {
      ...
      REFUND_PROVIDER_NOT_CONFIGURED: tr(
        "Возвраты через платёжный сервис сейчас отключены. Реальный возврат денег этой формой не выполняется.",
        "Төлем сервисі арқылы қайтару қазір өшірілген. Бұл нысан нақты ақшаны қайтармайды.",
      ),
      SANDBOX_FORBIDDEN_IN_PRODUCTION: tr(
        "Тестовые платёжные операции недоступны на рабочем сайте.",
        "Жұмыс сайтында тестілік төлем әрекеттері қолжетімсіз.",
      ),
      REFUND_AMOUNT_EXCEEDS_REMAINING: tr(
        "Сумма превышает доступный остаток возврата. Обновите данные заказа и проверьте сумму.",
        "Сома қайтаруға қолжетімді қалдықтан асады. Тапсырыс деректерін жаңартып, соманы тексеріңіз.",
      ),
      ORDER_NOT_REFUNDABLE: tr(
        "Для этого заказа возврат сейчас недоступен. Обновите его финансовое состояние.",
        "Бұл тапсырыс бойынша қайтару қазір қолжетімсіз. Қаржылық күйін жаңартыңыз.",
      ),
      REFUND_PROVIDER_MISMATCH: tr(
        "Этот платёж не поддерживает возврат в текущем тестовом режиме. Поступления по банковскому счёту сверяются отдельно.",
        "Бұл төлемді қазіргі тестілік режимде қайтару мүмкін емес. Банк шотына түскен қаражат бөлек салыстырылады.",
      ),
      REFUND_PAYMENT_MISMATCH: tr(
        "Данные заказа и подтверждённого платежа не совпадают. Возврат не выполнен; требуется финансовая сверка.",
        "Тапсырыс пен расталған төлем деректері сәйкес келмейді. Қайтару орындалмады; қаржылық салыстыру қажет.",
      ),
      IDEMPOTENCY_KEY_REQUIRED: tr(
        "Не удалось определить повторяемую операцию. Обновите страницу перед новым действием.",
        "Қайталанатын әрекетті анықтау мүмкін болмады. Жаңа әрекет алдында бетті жаңартыңыз.",
      ),
      IDEMPOTENCY_CONFLICT: tr(
        "Сведения повторного запроса отличаются от исходного. Обновите состояние и проверьте уже зарегистрированные операции.",
        "Қайта сұраудың деректері бастапқыдан өзгеше. Күйді жаңартып, тіркелген әрекеттерді тексеріңіз.",
      ),
  ...
  const statusLabel = (status: string) =>
    ({
      ...
      partially_refunded: tr("Частично возвращено", "Ішінара қайтарылды"),
...
export function safeLmsReturnTo(value: unknown, fallback = "/cabinet"): string {
  ...
  if (
    !/^\/(?:kk\/)?(?:cabinet(?:\/(?:organization|security|reminders))?|courses(?:\/[a-z0-9-]+)?|learn\/[a-zA-Z0-9_-]+(?:\/(?:exam|pre-test|confirm|success|failed))?|payment(?:\/[a-zA-Z0-9_-]+)?|certificates\/[a-zA-Z0-9_-]+|admin(?:\/(?:users|documents|incidents|support|analytics|leads|document-batches|programs(?:\/[a-zA-Z0-9_-]+)?))?)\/?$/.test(
      path,
    )
  )
    return fallback;

import { defineEventHandler, getCookie, getHeader, getRequestIP, readBody, setResponseStatus } from 'h3';
...
export default defineEventHandler(event => runWithRequestObservation(event, async () => {
  ...
  const result = await acceptLead(await readBody(event), getHeader(event, 'idempotency-key') || '', getCookie(event, analyticsCookieName));

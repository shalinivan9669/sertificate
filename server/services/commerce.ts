...
export async function getOrder(orderId: string, userId: string, db?: Db) {
  const row = await queryOne('SELECT * FROM orders WHERE id=? AND user_id=?', [orderId, userId], db);
...
async function verifiedPaymentWebhook(raw: string, signature: string) {
  ...
  return withTransaction(async tx => {
    ...
    if (['succeeded', 'refunded'].includes(payment.status)) return { received: true, ignored: true };
...
export async function refundOrder(orderId: string, actorId: string, reason: string) {
  ...
  return withTransaction(async tx => {
    const sandboxPayment = await queryOne('SELECT id FROM payments WHERE order_id=? AND provider=?', [orderId, 'sandbox'], tx);
    if (!sandboxPayment) fail(409, 'REFUND_PROVIDER_MISMATCH');
    const previous = await queryOne('SELECT id,status FROM refunds WHERE order_id=? AND status=?', [orderId, 'confirmed'], tx);
    if (previous) return previous;
    const order = await queryOne('SELECT * FROM orders WHERE id=?', [orderId], tx);
    if (!order || order.status !== 'succeeded') fail(409, 'ORDER_NOT_REFUNDABLE');
    const refundId = id();
    await execute('INSERT INTO refunds(id,order_id,status,amount_minor,reason,actor_id,created_at) VALUES(?,?,?,?,?,?,?)', [refundId, orderId, 'confirmed', order.amount_minor, reason, actorId, nowIso()], tx);
    await execute('UPDATE orders SET status=?,updated_at=? WHERE id=?', ['refunded', nowIso(), orderId], tx);
    await execute('UPDATE payments SET status=?,updated_at=? WHERE order_id=?', ['refunded', nowIso(), orderId], tx);
    ...
    return { id: refundId, status: 'confirmed', mode: 'sandbox' };
  });
}

export async function commerceOverview(userId: string) {
  const [orders, credentials] = await Promise.all([
    queryAll('SELECT * FROM orders WHERE user_id=? ORDER BY created_at DESC LIMIT 100', [userId]),
